# Introduction to Python using ArcGIS Online {.unnumbered}

Perhaps one of the fastest and easiest way to start with python as spatial data scientists for you is to create a notebook in ArcGIS Pro and use it to run some Python code. For this part you wont use the Python Enviroment you have installed, and you can use the Computer Lab, where ArcGIS Desktop is installed. To help you getting started and get familiar with the python Environment, we will use ArcGIS Desktop.

### Setting up the ArcGIS Pro Project

Using the computer lab.

1.  In Moodle download the file **PY4SA_Week5.ppkx** in the Week 5 to a location on your lab computer.

    ::: callout-note
    The file contains an ArcGIS Pro package that includes, maps, a database combined. **In this lesson the data is shown at specific folder. You can use a different folder, but be sure to adjust the paths in the instructions that follow.**
    :::
